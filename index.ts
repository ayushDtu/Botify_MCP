...
server.setRequestHandler(ListToolsRequestSchema, async () => {
  return {
    tools: [
      ...
      {
        name: "play-playlist",
        description: "Play a specific playlist on an active device",
        inputSchema: {
          type: "object",
          properties: {
            playlistId: {
              type: "string",
              description: "Spotify ID of the playlist to play",
            },
            deviceId: {
              type: "string",
              description: "Spotify ID of the device to play on (optional)",
...
Repeat: ${
      ...
      if (name === "play-playlist") {
        const { playlistId, deviceId } = PlayPlaylistSchema.parse(args);
        ...
        return {
          content: [
            {
              type: "text",
              text: `Started playing playlist with ID: ${playlistId}`,
            },
          ],
        };
      }
