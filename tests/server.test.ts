...
describe('MCP Server', () => {
  ...
  describe('Tool Implementations', () => {
    ...
    it('should provide playlist playback control', () => {
      // This just tests that the playlist playback handler can be properly set up
      const playPlaylistHandler = jest.fn().mockResolvedValue({
        content: [{ type: 'text', text: 'Playing playlist' }]
      });

      // Register the handler
      mockSetRequestHandler('play-playlist', playPlaylistHandler);

      // Verify it was registered
      expect(mockSetRequestHandler).toHaveBeenCalledWith('play-playlist', playPlaylistHandler);
    });
  });
});